...
export const getUserAssessmentDetailsAsManager = async ({
  assessmentId,
}: {
  assessmentId: string;
}): Promise<any> => {
  ...
  appLogger.info({ getUserAssessmentDetailsAsManager_scan_params: params });
  return scan<AssessmentDocument[]>(params).then((assessmentDocuments) => {
    if (!assessmentDocuments[0]) {
      return {};
    }
    return assessmentDocuments[0].assessmentDetails;
  });
};
